...
void Video::video_init(int width, int height, const Settings::Video& settings) {
	...
	glDeleteFramebuffers(2, rbo);
	glDeleteFramebuffers(2, mso);
	...
	auto samples = [&] {
		...
		case Settings::Video::EIGHT_X:	return 8;
		}
	}();
	...
	for(int i = 0; i < 2; i++) {
		// MSAA
		glGenFramebuffers(1, &mso[i]);
		glBindFramebuffer(GL_FRAMEBUFFER, mso[i]);
...
void Video::activate() {
	...
	glBindFramebuffer(GL_FRAMEBUFFER, mso[double_buffer]);
...
void Video::swap_buffers() {
	glBindFramebuffer(GL_READ_FRAMEBUFFER, mso[double_buffer]);
	glBindFramebuffer(GL_DRAW_FRAMEBUFFER, fbo[double_buffer]);
